...
while read -r bamfilepath; do
	job_id=$(sbatch --parsable --job-name="$(basename "$bamfilepath")" submit_preprocessbam.sh "$bamfilepath")
...
# right after scheduling Step 1 jobs:
#cp $TEPIPE_DIR/scripts/check_and_proceed.sh $BAM_fixed
#cd $BAM_fixed
#sbatch --dependency=afterok:${job_ids_str} check_and_proceed.sh
#cd ..
#########################################
#Step 1B: bamstat metrics cal
#########################################
cp $TEPIPE_DIR/scripts/submit_bam_stat.sh $TEPIPE_DIR/scripts/process_metrics.sh $base_dir/bamfilepaths.txt $BAM_fixed

cd $BAM_fixed
while read -r bamfilepath; do
    sbatch --dependency=afterok:$job_ids_str submit_bam_stat.sh "$bamfilepath"
