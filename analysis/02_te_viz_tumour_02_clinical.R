...
init_module_sink(paste0(plot_dir, "counts_cohort/"), "CLINICAL")
...
for (i in seq_along(types)) {
  ...
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_wilcox_aff_", ifelse(is.na(types[i]), "all", types[i]), ".png"), plot=plots_wilcox_aff[[i]], width = 3, height = 5)
}
...
plots_lm_aff <- lapply(seq_along(types), function(i) {
  ...
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_lm_aff_", ifelse(is.na(types[i]), "all", types[i]), ".png"), plot=p, width=5, height=5)
  p
})
...
ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_bootstrap_aff_dist_total.png"), plot=boot_aff$dist_plot, width=3, height=5)
titled_print(boot_aff$pval_plot, "Bootstrap test (affected, total) - P-value Convergence Plot")
ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_bootstrap_aff_pval_total.png"), plot=boot_aff$pval_plot, width=3, height=5)
...
for (i in seq_along(types)) {
  ...
  titled_print(plot_count_kruskal_nogroup(te_aff_t, column="tumor_type", x_lab="Tumour type", y_lab = "Repeat count", type=types[i], chr=NA, min=3, log_scale = TRUE),
               paste0("Kruskal (no group) by tumour type (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_tt_aff_", ifelse(is.na(types[i]), "all", types[i]), ".png"),
...
for (i in seq_along(types)) {
  ...
  titled_print(plot_count_kruskal_nogroup(te_kics_t, column="tumor_type", x_lab="Tumour type", y_lab="Repeat count", type=types[i], chr=NA, min=3, log_scale=TRUE, breaks=c(10, 100, 1000, 10000, 100000)),
               paste0("Kruskal (no group) by tumour type (KICS, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_tt_kics_", ifelse(is.na(types[i]), "all", types[i]), ".png"),
...
for (i in seq_along(types)) {
  ...
  titled_print(plot_count_kruskal_nogroup(te_lfs_t, column="tumor_type", x_lab="Tumour type", y_lab="Repeat count", type=types[i], chr=NA, min=3, log_scale=TRUE),
               paste0("Kruskal (no group) by tumour type (LFS, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_tt_lfs_", ifelse(is.na(types[i]), "all", types[i]), ".png"),
...
cat("\n===== BY TUMOUR CLASS (AFF) =====\n")
for (i in seq_along(types)) {
  write_output(quote(plot_count_kruskal_nogroup(te_aff_t, column="tumor_class", x_lab="Tumour class", y_lab="Repeat count", type=types[i], chr=NA, min=3, log_scale=TRUE)),
               paste0("Kruskal (no group) by tumour class (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  titled_print(plot_count_kruskal_nogroup(te_aff_t, column="tumor_class", x_lab="Tumour class", y_lab="Repeat count", type=types[i], chr=NA, min=3, log_scale=TRUE),
               paste0("Kruskal (no group) by tumour class (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_tc_aff_", ifelse(is.na(types[i]), "all", types[i]), ".png"),
         plot=plot_count_kruskal_nogroup(te_aff_t, column="tumor_class", x_lab="Tumour class", y_lab="Repeat count", type=types[i], chr=NA, min=3, log_scale=TRUE), width=9, height=5)
}

cat("\n===== BY TUMOUR CLASS (KICS) =====\n")
for (i in seq_along(types)) {
  write_output(quote(plot_count_kruskal_nogroup(te_kics_t, column="tumor_class", x_lab="Tumour class", y_lab="Repeat count", type=types[i], chr=NA, min=3, log_scale=TRUE, breaks=c(10, 100, 1000, 10000, 100000))),
               paste0("Kruskal (no group) by tumour class (KICS, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  titled_print(plot_count_kruskal_nogroup(te_kics_t, column="tumor_class", x_lab="Tumour class", y_lab="Repeat count", type=types[i], chr=NA, min=3, log_scale=TRUE, breaks=c(10, 100, 1000, 10000, 100000)),
               paste0("Kruskal (no group) by tumour class (KICS, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_tc_kics_", ifelse(is.na(types[i]), "all", types[i]), ".png"),
         plot=plot_count_kruskal_nogroup(te_kics_t, column="tumor_class", x_lab="Tumour class", y_lab="Repeat count", type=types[i], chr=NA, min=3, log_scale=TRUE, breaks=c(10, 100, 1000, 10000, 100000)), width=9, height=5)
}

cat("\n===== LINEAR MODEL BY TUMOUR CLASS (AFF) =====\n")
for (i in seq_along(types)) {
  y_label <- ifelse(is.na(types[i]), "Total TE count", paste0(types[i], " count"))
  p <- plot_count_lm(te_aff_t, min_samples=3, residuals=FALSE, log_scale=TRUE, group="tumor_class", covariates=covar_med, x_lab="Tumour class", y_lab=y_label, type=types[i], chr=NA)
  write_output(quote(plot_count_lm(te_aff_t, min_samples=3, residuals=FALSE, log_scale=TRUE, group="tumor_class", covariates=covar_med, x_lab="Tumour class", y_lab=y_label, type=types[i], chr=NA)),
               paste0("Linear model by tumour class (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  titled_print(p, paste0("Linear model by tumour class (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_lm_tc_aff_", ifelse(is.na(types[i]), "all", types[i]), ".png"), plot=p, width=9, height=5)
}

cat("\n===== LINEAR MODEL BY TUMOUR CLASS (KICS) =====\n")
for (i in seq_along(types)) {
  y_label <- ifelse(is.na(types[i]), "Total TE count", paste0(types[i], " count"))
  p <- plot_count_lm(te_kics_t, min_samples=3, residuals=FALSE, log_scale=TRUE, group="tumor_class", covariates=covar_med, x_lab="Tumour class", y_lab=y_label, type=types[i], chr=NA)
  write_output(quote(plot_count_lm(te_kics_t, min_samples=3, residuals=FALSE, log_scale=TRUE, group="tumor_class", covariates=covar_med, x_lab="Tumour class", y_lab=y_label, type=types[i], chr=NA)),
               paste0("Linear model by tumour class (KICS, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  titled_print(p, paste0("Linear model by tumour class (KICS, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_lm_tc_kics_", ifelse(is.na(types[i]), "all", types[i]), ".png"), plot=p, width=9, height=5)
}
...
write.csv(te_aff_top10, paste0(plot_dir, "counts_clinical_lfs/te_aff_top10_samples.csv"), row.names = FALSE, quote = FALSE)
...
if (nrow(outlier_samples) > 0) {
  ...
  write.csv(outlier_samples, paste0(plot_dir, "counts_clinical_lfs/te_aff_outliers_by_tumor_type.csv"), row.names = FALSE, quote = FALSE)
...
for (i in seq_along(types)) {
  ...
  titled_print(plot_count_tt(te_aff_t, chr=NA, type=types[i], group="TP53_status", min=3, y_lab="Repeat count", legend_title=x_tp53, log_scale=TRUE),
               paste0("Count by tumour type and TP53_status (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_tt_tp53_", ifelse(is.na(types[i]), "all", types[i]), ".png"),
...
cat("\n===== KRUSKAL TEST BY ANCESTRY =====\n")
for (i in seq_along(types)) {
  p <- plot_count_kruskal(te_aff_t, chr=NA, type=types[i], group="predicted_ancestry_thres", x_lab="Predicted Ancestry", y_lab="Repeat count", log_scale=TRUE, fill_palette=color_palette_7)
  write_output(quote(plot_count_kruskal(te_aff_t, chr=NA, type=types[i], group="predicted_ancestry_thres", x_lab="Predicted Ancestry", y_lab="Repeat count", log_scale=TRUE, fill_palette=color_palette_7)),
               paste0("Kruskal by ancestry (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  titled_print(p, paste0("Kruskal by ancestry (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_kruskal_ancestry_", ifelse(is.na(types[i]), "all", types[i]), ".png"), plot=p, width=9, height=5)
}

cat("\n===== LINEAR MODEL BY ANCESTRY =====\n")
for (i in seq_along(types)) {
  y_label <- ifelse(is.na(types[i]), "Total TE count", paste0(types[i], " count"))
  p <- plot_count_lm(te_aff_t, min_samples=5, residuals=FALSE, group="predicted_ancestry_thres", log_scale=TRUE, covariates=covar_no_ancestry, x_lab="Predicted Ancestry", y_lab=y_label, type=types[i], chr=NA, fill_palette=color_palette_7)
  write_output(quote(plot_count_lm(te_aff_t, min_samples=5, residuals=FALSE, group="predicted_ancestry_thres", log_scale=TRUE, covariates=covar_no_ancestry, x_lab="Predicted Ancestry", y_lab=y_label, type=types[i], chr=NA, fill_palette=color_palette_7)),
               paste0("Linear model by ancestry (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  titled_print(p, paste0("Linear model by ancestry (affected, type=", ifelse(is.na(types[i]), "all", types[i]), ")"))
  ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_lm_ancestry_", ifelse(is.na(types[i]), "all", types[i]), ".png"), plot=p, width=9, height=5)
}
...
cat("\n===== LFS STATUS BY COHORT =====\n")
# Plot TE counts comparing LFS status by cohort
for (i in seq_along(types)) {
  type_label <- ifelse(is.na(types[i]), "all", types[i])
  y_label <- ifelse(is.na(types[i]), "Total TE count", paste0(types[i], " count"))
  combination <- ifelse(is.na(types[i]), "total", types[i])

  # Filter to samples with valid cohort information
  te_all_cohort <- te_all_t %>% filter(!is.na(cohort))

  if (nrow(te_all_cohort) > 0 && length(unique(te_all_cohort$cohort)) >= 2) {
    p <- plot_count_kruskal(te_all_cohort, type=types[i], chr=NA, group="cohort",
                            x_lab="Cohort", y_lab=y_label, log_scale=TRUE)
    write_output(quote(plot_count_kruskal(te_all_cohort, type=types[i], chr=NA, group="cohort",
                                          x_lab="Cohort", y_lab=y_label, log_scale=TRUE)),
                 paste0("TE count by cohort (type=", type_label, ")"))
    titled_print(p, paste0("TE count by cohort (type=", type_label, ")"))
    ggsave(paste0(plot_dir, "counts_clinical_kics/te_count_by_cohort_", type_label, ".png"), plot=p, width=6, height=5)
  } else {
    cat("Skipping cohort plot for type", type_label, "- insufficient cohort data\n")
  }
}

cat("\n===== LFS TE COUNT BY TUMOR TYPE - COLORED BY COHORT =====\n")
# Use te_lfs_t data, plot by tumor type with cohort as coloring variable
# Exclude KiCS (not in LFS), keep only tumor types with at least 3 total samples
te_lfs_valid <- te_lfs_t %>%
